...
bool WaitPosesCB(vr_src::TrackedDevicePose_t* pRenderPoseArray, uint32_t unRenderPoseArrayCount, vr_src::TrackedDevicePose_t* pGamePoseArray, uint32_t unGamePoseArrayCount)
{
	...
	if ((g_rightGrabber.state != Grabber::State::HELD && g_rightGrabber.pulledObject.handle == *g_invalidRefHandle) && (g_leftGrabber.state != Grabber::State::HELD && g_leftGrabber.pulledObject.handle == *g_invalidRefHandle)) {
